...
var CrudComponent = /** @class */ (function () {
    function CrudComponent(lms, api, dialog) {
        ...
        this.unsubGetEmpCSV = this.api.emitgetEmpCSV.subscribe(function (e) {
            _this.emplCSV = e;
            var i, j, k;
            for (i = 0; i < _this.emplCSV.length; i++) {
                for (j = 1; j <= 12; j++) {
                    k = j;
                    if (j < 10) {
                        j = '0' + j;
                    }
                    _this.temp1.push(_this.emplCSV[i][j]);
                    j = k;
                }
                _this.employeeLeave.push(_this.temp1);
                _this.temp1 = [];
            }
        });
    }
...
var DashboardComponent = /** @class */ (function () {
    ...
    DashboardComponent.prototype.createCalendar = function (month) {
        ...
        setTimeout(function () {
            ...
            _this.api.postEOLBSDate(_this.workingDays); // lms/count
...
var ApiService = /** @class */ (function () {
    ...
    ApiService.prototype.login = function (uname, pwd) {
        var _this = this;
        this.uid = uname;
        var tmp = { email_id: uname, password: pwd };
    ...
    ApiService.prototype.signup = function (name, password, email_id) {
        var _this = this;
        var tmp = { name: name, password: password, email_id: email_id };
        var data = JSON.stringify(tmp);
        return new Promise(function (resolve) {
            _this.http.post(_this.URL + 'lms/addAdmin', data)
                .map(function (res) { return res.json(); })
                .subscribe(function (response) {
                if (response.success) {
                    _this.snackBars('Admin added', 'Kindly login');
                }
                else
                    _this.snackBars(response.message, response.success);
                resolve(true);
            ...
            this.http.post(this.URL + 'lms/api', data, this.opts)
                ...
                .subscribe(response => {
                    ...
                    if (response.success) {
                        ...
                        // does not refresh after response
                    } else this.snackBars("Api", response.success)
                    resolve(true)
